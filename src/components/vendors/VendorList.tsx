...
export const VendorList: React.FC = () => {
  ...
  const souvenirVendors = vendors.filter(
    (v) => v.type === "Souvenir Vendor",
  ).length;
  const customizationVendors = vendors.filter(
    (v) => v.type === "Customization Vendor",
  ).length;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Manage Unboxie's supplier network.
          </p>
        </div>
      ...
      {filteredVendors.length === 0 ? (
        <div className="p-12 text-center rounded-2xl bg-white border border-dashed border-slate-300">
          ...
          <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
            Try adjusting your search criteria or add a new vendor to the
            network.
          </p>
          <button
            onClick={() => {
