...
export const VendorDetailPage: React.FC<VendorDetailPageProps> = ({
  ...
  const [activeTab, setActiveTabLocal] = useState<
    "overview" | "products" | "orders" | "incidents"
  >("overview");
    ...
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5 self-start sm:self-auto flex-wrap">
          ...
          <button
            onClick={() =>
              setCurrentView({ type: "vendor-form", id: vendor.id })
            }
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5">
        ...
        <div className="p-4 rounded-2xl bg-white border border-slate-200/80 shadow-subtle flex flex-col justify-between">
          ...
          <div>
            ...
            <p
              ...
              title={`(${performance.issueCount} incidents ÷ ${performance.totalSourcedItems} items) × 100`}
            >
              {performance.issueCount} incident
              {performance.issueCount === 1 ? "" : "s"} /{" "}
              {performance.totalSourcedItems} items
            </p>
          </div>
        </div>
        ...
        {/* Quality Loss & Liability Split (Merged) */}
        <div className="p-4 rounded-2xl bg-white border border-slate-200/80 shadow-subtle flex flex-col justify-between">
          ...
          <div>
            ...
            <div className="mt-2 pt-2 border-t border-slate-100 flex items-center justify-between text-xs font-semibold gap-2">
              <span className="text-emerald-700">Vendor: <strong>{formatNaira(performance.costCoveredByVendor)}</strong></span>
              <span className="text-rose-600">Unboxie: <strong>{formatNaira(performance.costCoveredByUnboxie)}</strong></span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-subtle overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-6 pt-4 bg-slate-50/50">
          <div className="flex items-center space-x-2">
            ...
            >
              ...
              <span>
                Supplied Catalog Items ({products.length + packaging.length})
              </span>
            </button>
            ...
            {incidents.length === 0 ? (
              <div className="p-8 text-center rounded-xl bg-emerald-50/50 border border-dashed border-emerald-200 text-emerald-800">
                ...
                <p className="text-xs text-emerald-600/90 mt-0.5">
                  This vendor has provided quality products with zero registered
                  complaints.
                </p>
              </div>
            ) : (
        ...
        {activeTab === "overview" && (
          <div className="p-6 space-y-4">
            {/* Overview Meta Banner */}
            <div className="p-3.5 rounded-xl bg-slate-100/70 border border-slate-200/80 text-xs font-semibold text-slate-700 flex items-center gap-2 flex-wrap">
              <span className="text-slate-900 font-bold">{vendor.type}</span>
              <span>•</span>
              <span className="text-slate-700">{vendor.source} Channel</span>
              <span>•</span>
              <span className="text-slate-500">Joined {formatDate(vendor.createdAt)}</span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2 text-xs">
                ...
                <p className="text-slate-600 flex items-center gap-1.5 mt-2">
                  ...
                  <span>
                    {[vendor.cityLga, vendor.state].filter(Boolean).join(", ")}
                  </span>
                </p>
                {vendor.address && (
